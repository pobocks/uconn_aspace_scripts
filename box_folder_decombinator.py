...
def sort_fn(row):
    return (row['resource_id'], row['candidate_indicator'])

def fetch_existing_aos(container_id):
    uri = f'/repositories/2/top_containers/{container_id}'

    # returns hash of container_id -> jsons
    return {container_id:json.loads(result['json'])
            for result
...
if __name__ == '__main__':
    ...
    with args.report as reportfile:
        ...
        for key, group in groupby(sorted(reader, key=sort_fn), key=sort_fn):
            log = asnake.logging.get_logger('uconn_box_folder_decombinator').bind(group_key=key)
            try:
                # we gotta do a bit of stuff with the group so turn it into a stable object
                group = list(group)
                log.info('processing containers', ids=[row['container_record_id'] for row in group], indicators=[row['container_indicator'] for row in group])
                ...
                candidate_ids = group[0]['candidate_container_ids'].split(',') if group[0]['candidate_container_ids'] else []
                ...
                if  number_of_candidates > 1:
                    ...
                    for cand_id, ao in victim_aos.items():
                        ...
                        cand_json = client.get(cand_uri).json()

                        replacement_instances = []
                        for instance in fresh_ao.get('instances', []):
                            # valid but not our boy, just add it
                            if validate_instance(instance, cand_uri):
                                replacement_instances.append(instance)
                            # IS our candidate, replace top_container ref and barcode if present, then add it
                ...
                # one candidate, fetch any existing sub containers and add these guys
                    ...
                    log.info('merging into existing container')
                ...
                # no candidate, create new container
                    ...
                    log.info('creating new top container')
                    ...
                    if not args.dry_run:
                        ...
                        log.warning("dry run can't simulate new TC creation scenario")
                        continue
